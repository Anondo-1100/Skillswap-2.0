...
import { useNavigate, useLocation, Link
...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setError('');
      const response: AuthResponse = await login(email.trim(), password);
      if (response.status === 'error') {
        if (response.code === 'auth/wrong-password') {
          setError('The password you entered is incorrect. Please try again.');
        } else if (response.code === 'auth/user-not-found') {
          setError('We couldn\'t find an account with this email address. Please check your email or sign up.');
